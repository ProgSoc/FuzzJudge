...
export async function attempts(params: { team: number; prob: string }): Promise<number> {
...
export async function solved(params: { team: number; prob: string }): Promise<boolean> {
...
export async function solvedSet(params: { team: number }): Promise<Set<string>> {
  const solved = await db
    .selectDistinct({ prob: submissionTable.prob })
    .from(submissionTable)
    .where(and(eq(submissionTable.team, params.team), eq(submissionTable.ok, true), isNotNull(submissionTable.prob)));
...
export async function manualJudge(id: number, ok: boolean) {
  ...
  return await db.update(submissionTable).set({ ok }).where(eq(submissionTable.id, id)).returning();
}
...
export async function postSubmission(
  ...
  if (resubmit && ok) {
    ...
    const [id] = await db
      ...
      .where(and(eq(submissionTable.prob, prob), eq(submissionTable.team, team), eq(submissionTable.ok, true)))
      .returning();
...
export async function getSubmissionOut(id: number): Promise<string | undefined> {
...
export async function getSubmissionCode(id: number): Promise<string | undefined> {
...
export async function getSubmissionVler(id: number): Promise<string | undefined> {
