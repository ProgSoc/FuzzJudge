...
const loadIcon = async (metaPath: string): Promise<Icon> => {
  const icon: Icon = TOML.parse(await Bun.file(joinPath(ICONS_DIR, metaPath)).text());
...
(async () => {
  for await (const file of await fs.readdir(ICONS_DIR, { withFileTypes: true})) {
